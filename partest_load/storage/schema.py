...
class LoadTestRun(BaseModel):
    ...
    «МЕТОД /путь». Это то, чем эндпоинт опознаётся в семействе; по нему замер сводится
    с внешними данными о том же эндпоинте. `slug` — человекочитаемая подпись прогона, она
